...
function wpmu_activate_stylesheet() {
    ...
    <style type="text/css">
        ...
        span.h3 { padding: 0 8px; font-size: 1.3em; font-weight: bold; color: #fff; }
    </style>
    <?php
}
    ...
    <?php } else {
        ...
        if ( is_wp_error($result) ) {
            ...
            <?php if ( $url != network_home_url('', 'http') ) : ?>
                <p class="view"><?php printf( __('Your account is now activated. <a href="%1$s">View your site</a> or <a class="button" href="%2$s">Log in</a>'), $url, $url . 'wp-login.php' ); ?></p>
            <?php else: ?>
  <p class="view viewactive"><?php printf( __('Your account is now activated. <a class="button" href="%1$s">Log in</a> or go back to <a href="/the-wall">The Wall</a>.' ), network_site_url('wp-login.php', 'login'), network_home_url() ); ?></p>
            <?php endif;
        }
    }
    ?>
